...
public static class Example64_FlowOrchestrator
{
    ...
    steps:
      ...
      - goal: Give user the decision of the interview based on user's interview performance.
        plugins:
          - GiveInterviewFeedbackAndDecision
        requires:
          - problem_statement
          - solution_code_implementation
          - time_complexity
...
[JSON format]
{{
    ""solution_code_implementation"": <code implementation>
}}.
[END JSON format]
...
[Example]
    ...
    {Delimiter}
    {{
        ""solution_code_implementation"": ""def max_subarray_sum(arr):
                            n = len(arr)
                            max_sum = arr[0]
                            current_sum = arr[0]
                            for i in range(1, n):
...
";
            ...
            SKContext context)
        {
            ...
            if (match.Success)
            {
                ...
                context.Variables["solution_code_implementation"] = solutionJson["solution_code_implementation"].Value<string>();
    ...
    {
        private const string Problem = "problem_statement";
        private const string Solution = "solution_code_implementation";
        private const string Delimiter = "```";
...
";
        ...
        [SKName("PromptTimeAndSpaceComplexityAnalysis")]
        public async Task<string> PromptTimeAndSpaceComplexityAnalysisAsync(
            [SKName("problem_statement")][Description("The problem statement generated by the interviewer")] string problem_statement,
            [SKName("solution")][Description("The full code implementation given by the user")] string solution_code_implementation,
            SKContext context)
        {
            //Console.WriteLine("<======= Creating PromptTimeAndSpaceComplexityAnalysis chat =======>\n");
            var chat = this._chat.CreateNewChat(SystemPrompt);
            chat.AddUserMessage(Goal);
    ...
    public sealed class GiveInterviewFeedbackAndDecision
    {
...
";
        ...
        public GiveInterviewFeedbackAndDecision(IKernel kernel)
        {
            this._chat = kernel.GetService<IChatCompletion>();
            this._chatRequestSettings = new OpenAIRequestSettings
            {
        ...
        [SKName("GiveInterviewDecisionAndFeedback")]
        public async Task<string> GiveInterviewDecisionAndFeedbackAsync(
            [SKName("solution")][Description("The full implementation given by the user")] string solution_code_implementation,
            ...
            SKContext context)
        {
            //Console.WriteLine("<======= Creating GiveInterviewFeedbackAndDecision chat =======>\n");
            ...
            var result = await this._chat.GenerateMessageAsync(chat, this._chatRequestSettings).ConfigureAwait(false);
...
//User: I will initialize max_sum and current_sum to the first element of the list. Then, I will loop through the list, adding each element to current_sum. If current_sum is greater than max_sum, I will update max_sum. If current_sum is negative, I will reset it to zero. After processing all elements, I will return max_sum, which represents the maximum sum of any contiguous subarray within the list.
//Assistant: ["That\u0027s a great approach! Can you go ahead and implement the solution in your preferred programming language?"]
//User: My solution is:
//def max_subarray_sum(arr):
// n = len(arr)
// max_sum = arr[0]
...
//User: Yes.
//Assistant: ["What is the time and space complexity of your solution\u0027s code implementation?"]
//User: I think the time complexity is O(n) since I iterate through the array exactly once and the space complexity is O(1) since there's no extra space used.
